...
const App: React.FC = () => {
  ...
  const handleDiscover = async (id: string) => {
    ...
    if (p) {
      setIsLoading(true);
      // Triggering a re-load of sheets by updating the projects state slightly or just re-running the logic
      // Since our useEffect for loadSheets depends on 'projects', we can just re-fetch projects
      const globalProjects = await fetchGlobalProjects();
      setProjects(globalProjects);
      setIsLoading(false);
    }
  ...
  return (
    <div className="flex h-screen bg-[#020617] text-slate-200 overflow-hidden relative">
      ...
      {showProjectManager && <ProjectManager projects={projects} selectedProjectIds={combinedSelectedProjectIds} userRole="desicrew" onAdd={addProject} onUpdate={updateProject} onDelete={deleteProject} onDiscover={handleDiscover} onSelect={handleSelectProject} onClose={() => setShowProjectManager(false)} />}
